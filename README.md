...
책상 위에 두면 업무용 리마인더처럼, 침대 협탁에 두면 자기 전 마지막으로 확인하는 루틴처럼 쓸 수 있다. 어디 놓아도 크기가 커피잔 하나 정도라 자리를 차지하지 않으면서, 매일 무심코 지나치다 한 번씩 눈이 가는 존재감은 남긴다. 화면은 표정 4가지(위험·주의·양호·안심)로만 말하기 때문에 숫자를 해석할 필요 없이 직관적으로 상태를 파악할 수 있다.
...
1. `train_credit_risk_model.py` — PyTorch MLP로 가계 신용 리스크 모델 학습 (AUC 0.801)
2. `household_face_score.py` — 학습된 모델로 현재 점수(0~100) 계산 → `score.txt`
3. `robot_face_eyes.py` / `robot_main.py` — 점수를 표정으로 변환해 OLED에 출력, 평소엔 랜덤 깜빡임으로 생동감 부여
